...
class NystatespiderSpider(scrapy.Spider):
    ...
    output = 'nystate.csv'
    open(output, 'w').close()
    header = ['AgentName', 'Area', 'Address', 'PhoneNo1']
    ...
    def get_urls(self, response):
        ...
        with open(self.output, "a", newline="", encoding='utf-8') as f:
            ...
            all_accounts = len(item_urls)

            agentNames = response.xpath("//a[contains(@class, 'ActionLink')]/text()").extract()

            counties = response.xpath("//table[@class='jQDataTable']//tbody//tr/td[2]/text()").extract()

            locations = response.xpath("//table[@class='jQDataTable']//tbody//tr/td[3]/text()").extract()

            Phones = response.xpath("//table[@class='jQDataTable']//tbody//tr/td[4]/text()").extract()

            for i in range(1, all_accounts + 1):
                agentName = response.xpath("(//a[contains(@class, 'ActionLink')])[{}]/text()".format(i)).get()

                county = response.xpath("//table[@class='jQDataTable']//tbody/tr[{}]/td[2]/text()".format(i)).get()

                location = response.xpath("//table[@class='jQDataTable']//tbody/tr[{}]/td[3]/text()".format(i)).get()

                phone = response.xpath("//table[@class='jQDataTable']//tbody/tr[{}]/td[4]/text()".format(i)).get()

                print("-=-=-----------------------------------------")
                print("AngentName------------> : ", agentName)
                print("County----------------> : ", county)
                print("Location--------------> : ", location)
                print("Phone-----------------> : ", phone)
 
                writer.writerow([agentName, county, location, phone])
